...
def listar_seguindo(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    for s in seguidos:
        ...
        resultado.append(FollowedPoliticianResponse(
            ...
            # Expõe o external_id para o frontend poder sincronizar
...
def seguir_politico(external_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Busca pelo external_id (ID da Câmara) em vez do ID interno
...
def deixar_de_seguir(external_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Busca o Politician pelo external_id para obter o id interno
    p = db.query(Politician).filter(Politician.external_id == external_id).first()
    if not p:
        raise HTTPException(status_code=404, detail="Político não encontrado")
